#!/usr/bin/env python3
"""
Predict NMR tensors for He atoms from XYZ files using the matten model.
...
This script reads an XYZ file containing multiple structures with He and C atoms,
converts them to PyMatGen Structure objects, and predicts NMR tensors for He atoms.
...
def predict_nmr_tensors_incremental(structures, model_path, checkpoint_name, output_file, 
    ...
    """
    Predict NMR tensors for all He atoms in the structures with incremental saving.
    ...
    for i in range(start_from, len(structures)):
        ...
        try:
            ...
            # Find He atom indices
            he_indices = []
            for j, site in enumerate(structure.sites):
                if str(site.specie) == 'He':
                    he_indices.append(j)

            if not he_indices:
                print(f"  No He atoms found in structure {i + 1}")
                # Save progress and continue
                save_progress(progress_file, i + 1)
                structures_processed_in_batch += 1
                continue

            print(f"  Found {len(he_indices)} He atoms at indices: {he_indices}")
            ...
            # Extract tensors for He atoms only
            structure_predictions = 0
            for he_idx in he_indices:
                if he_idx < len(tensors):
                    tensor = tensors[he_idx]

                    # Calculate isotropic shielding (sigma_iso)
                    sigma_iso = (tensor[0][0] + tensor[1][1] + tensor[2][2]) / 3.0

                    print(f"    He atom {he_idx}: sigma_iso = {sigma_iso:.4f}")

                    result = {
                        'structure_id': structure_info['id'],
                        'atom_index': he_idx,
                        'element': 'He',
                        'x': structure_info['coordinates'][he_idx][0],
                        'y': structure_info['coordinates'][he_idx][1],
                        'z': structure_info['coordinates'][he_idx][2],
                        'sigma_iso': sigma_iso,
                    ...
                    print(f"    Warning: Tensor not found for He atom {he_idx}")
...
def print_final_statistics(output_file):
    ...
    try:
        ...
        print(f"Total He predictions: {len(df)}")
...
def main():
    ...
    try:
        ...
        print(f"Generated {total_predictions} new He NMR predictions")
