...
With NequIP and Torch installed, and Python 3.10+, PyTorch 2.2+, and a CUDA-compatible GPU available, the training process of NequIP model may be started simply with the command
```nequip-train -cn config.yaml```

Doing the training on a HPC can be started by running the script
